...
impl<'info> Withdraw<'info> {
    pub fn withdraw(&mut self, amount: u64) -> Result<()> {
        //signer seeds to sign on behalf of the vault pda
        let signer_seeds: &[&[&[u8]]] = &[&[
            b"vault",
        ...
        //normal cpi call to withdraw sol from the vault
...
impl<'info> Close<'info> {
    pub fn close(&mut self) -> Result<()> {
        //aagain using signer seeds to sign the close instruction on behalf of the vault pda
        let signer_seeds: &[&[&[u8]]] = &[&[
            b"vault",
            self.vault_state.to_account_info().key.as_ref(),
            &[self.vault_state.vault_bump],
